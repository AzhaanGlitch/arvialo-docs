...
Dockerfile includes step-by-step instructions to build a Docker image that Arvialo uses to deploy your application or website. 

The Dockerfile build pack allows you to use your own Dockerfile to deploy your application, you have complete control over how your application is built and deployed on Arvialo.
...
Arvialo defaults to using Nixpacks. Click the Nixpacks option and select **Dockerfile** as your build pack from the dropdown menu.
...
Arvialo sets the default port to 3000, so if your application listens on a different port, update the port number on the network section.
...
You can manage your environment variables from the Arvialo UI. 
